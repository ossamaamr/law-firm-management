...
| P1-005 | P1 | DOCUMENT LIFECYCLE | Malware/versioning/retention غير مثبتة | documents/storage | policy الأساسية فقط | versioning، scanning، retention | scanner/storage provider | أدلة ضارة/فقدان نسخ | IMPLEMENTED fail-closed repository؛ `scanStatus` المفقود والجديد defaultه `pending`، migration `0012` تعيد legacy `clean` إلى pending، وروابط التنزيل محجوبة لغير clean. ما زال اعتماد scanner الخارجي مطلوبًا لإغلاق البند الإنتاجي |
...
| P1-007 | P1 | FINANCIAL INTEGRITY | Fake payment وغياب ledger/transactions | external APIs, invoices | adapters placeholder | إما تعطيل واضح أو تكامل مالي حقيقي idempotent | مزود دفع واعتماد تجاري | أخطاء مالية | IMPLEMENTED repository ledger / external provider blocked؛ `ledgerEntries` append-only وtenant-scoped مع idempotency key فريد، و`ledger.list` مقيد بالأدوار المالية، و`ledger.recordInvoiceIssued` يربط الفاتورة بقيد obligation idempotent بعد تحقق Tenant. لا يُحتسب البند كاملًا حتى تُربط تسوية دفع حقيقية أو يُعتمد ledger كبديل تشغيلي كامل |
| P1-008 | P1 | DATABASE INTEGRITY | FK/index/unique/transaction coverage غير كاملة | `drizzle/schema.ts` | اعتماد على فحوص تطبيقية فقط | فهارس وقيود مركبة ومراجعة transactions | MySQL migration review | orphan/N+1 | IMPLEMENTED repository؛ migration `0011_core_relationship_fks.sql` و`docs/MERSAD_DB_PREFLIGHT.sql` يضيفان ويفحصان FK والعلاقات العابرة للمكاتب. التطبيق الفعلي على MySQL معتمد ما زال مطلوبًا قبل إغلاق بند الإنتاج |
...
| F-008 | High | audit insert غير ذري مع العملية وفشله لا يفشل mutation | `server/activity.service.ts`, `server/db.ts`, `drizzle/schema.ts` | PARTIAL — أضيف auditOutbox دائم مع eventKey وretry وprojection idempotency، وربط إنشاء القضية داخل transaction؛ بقية mutations الحساسة تحتاج الربط نفسه | يمنع release sign-off حتى إكمال الذرية لكل العمليات الحساسة |
...
> **تحديث القسم الخامس:** أضيف `/health/ready` configuration gate في production، ووُسع `docs/MERSAD_DB_PREFLIGHT.sql` ليغطي migrations 0014–0016 والعزل المالي والتدقيقي وoutbox، وأضيف `docs/MERSAD_PRODUCTION_READINESS.md`. نجحت check/test/build/drizzle check، بنتيجة 154 اختبارًا ناجحًا و30 متخطيًا، لكن `pnpm audit --audit-level=high` ما زال فاشلًا بـ6 High، ولا يوجد MySQL تشغيلية متاحة لتطبيق migrations وتشغيل الاختبارات المتخطية. لذلك يبقى F-009 مفتوحًا وقرار الإطلاق `NOT READY`. النسبة التاريخية 26/28 ليست النسبة الحالية بعد إضافة Findings الجنائية.
